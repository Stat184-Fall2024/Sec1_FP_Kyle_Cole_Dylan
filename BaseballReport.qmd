...
For our analysis, we will be using the *Baseball Databank* dataset found on Kaggle [@opensourcesports]. This dataset has been adapted from the Sean Lahman Baseball Dataset [@lahman]. Sean Lahman is a well-respected journalist and data analyst with a passion for baseball history.
...
Our calculations are based on Baseball Reference's definitions [@warexpl] for $WAR$ (aka $bWAR$). Metrics such as $WAR$ can be calculated differently depending on the use case.

According to Baseball Reference's definition of $bWAR$ for a position player [@position]:
...
WAR is calculated differently for pitchers than it is position players. According to Baseball Reference's definition of WAR for a pitcher [@pitcher]:
...
For more information on baseball statistics, please refer to Baseball Reference [@warexpl].
...
This research question aims to uncover the salary range, salary spread, and the skewness of the adjusted annual salary distribution. To explore this question, we have created a density plot for the adjusted annual salaries.
