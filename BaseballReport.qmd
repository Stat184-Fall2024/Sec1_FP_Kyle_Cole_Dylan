...
top_10_over_table <- top_10_over_performers %>%
  ...
  kable(col.names = c("Year", "Player", "Position", "WAR", "Salary ($USD)", "WAR / $"),
        caption = "Top 10 Over Performers (Lowest WAR per Dollar)",
...
top_10_under_table <- top_10_under_performers %>%
  select(yearID, name, POS, WAR, salary_adj, WAR_per_dollar) %>%
  kable(col.names = c("Year", "Player", "Position", "WAR", "Salary ($USD)", "WAR / $"),
        caption = "Top 10 Under Performers (Lowest WAR per Dollar)",
        digits = c(0, 0, 0, 2, 0, 2)) %>%
  kable_styling(bootstrap_options = c("striped", "condensed", "bordered"))
```
...
Baseball statistics provide a way to quantify an individual player's or team's success by comparing them to other players and teams in the league. There are a wide range of baseball statistics used in analysis of professional baseball. Our analysis will be focusing on two primary categories of statistics: batting statistics and pitching statistics.

### Batting Statistics

For batting statistics, our analysis will focus on four metrics we consider to be the most indicative of a player's success at the plate: wins above replacement (WAR), batting average (BA), on base plus slugging (OPS), and normalized on base plus slugging (OPS+).
...
Describe how data was transformed from raw form to feature rich data.
...
Kaggle baseball databank

Sean Lahman baseball dataset

Bureau of Labor Statistics CPI dataset

Baseball reference WAR blog
